'''Hub implementing the Direct Connect protocol over IRC'''

import os, signal, socket, sys, time, logging, errno
...
class DCHub(object):
    ...
    def get_remote_user(self, nick):
        for user in self.remote_users:
            if user.nick.lower() == nick.lower(): return user
        return None
    
    def send_message(self, msg): self.local_user.sendmessage(msg)
    # 
    ...
    def handlewritesockets(self, writesockets):
        ...
        for id in writesockets:
            ...
            try:
                ...
                print 'Data sent to %s: %r' % (user.idstring, user.outgoing[:sentsize])
